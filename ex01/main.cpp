...
/*   Updated: 2025/10/07 13:20:43 by yidemir          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	str_is( std::string str, char type )
{
	unsigned char	c;
	...
	{
		c = str[i];
		if (type == 'a' && !(std::isalpha(c) || c == ' '))
			return (false);
		else if (type == 'n' && !(std::isalpha(c) || std::isdigit(c) || c == ' ' || c == '-' || c == '_'))
			return (false);
		else if (type == 'd' && !std::isdigit(c))
			return (false);
	}
	return (true);
}
...
static std::string	get_contact_field( const char *name, char type )
{
	...
	while (line.empty())
	{
		...
		else if(type == 'd' && !str_is(line, 'd'))
		{
			std::cout << name << " should be only numbers." << std::endl;
			line = "";
		}
		else if (type == 'a' && !str_is(line, 'a'))
		{
			std::cout << name << " should be only ascii alphabets or space." << std::endl;
			line = "";
		}
		else if (type == 'n' && !str_is(line, 'n'))
		{
			std::cout << name << " should be only [a-zA-Z0-9 _-]." << std::endl;
			line = "";
		}
	}
	return (line);
...
static void	handle_add( PhoneBook *phonebook )
{
	...
	nickname = get_contact_field("nickname", 'n');
