...
/*   Updated: 2025/07/16 09:02:43 by yidemir          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parser(t_shell *sh)
{
	...
	if (!token)
		return ;
	if (token->type == T_PIPE || is_operator(token->type))
		return ((void) syntax_err(sh, token->value));
	cmd = new_cmd(&sh->cmd_head);
	while (token)
	{
		cmd->and_op = (token->type == T_OPERATOR_AND);
		cmd->or_op = (token->type == T_OPERATOR_OR);
		...
		else if (token->type == T_PIPE || is_operator(token->type))
			cmd = new_cmd(&sh->cmd_head);
		token = token->next;
	}
}
